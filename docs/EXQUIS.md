...
**Shipped layouts** (`exquis-layouts/*.xqilayout`, generated by `tools/exquis_layouts.py`, also copied into the app's
`Documents/Intuitive Instruments/Exquis/Layouts` folder on this machine), for the keyboard on its side:
...
slots 1-3, close the window (that writes them to the keyboard), close the app.
