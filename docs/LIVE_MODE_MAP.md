...
| **DAW button** | toggles the **layout**: General DAW <-> LoopCanvas (only the pad modes differ). The watcher answers each press with a one-second pad sweep: green = General DAW, azure = LoopCanvas. The keyboard's screen cannot be written to in Live mode and keeps saying Ableton |
...
| # | General DAW layout | LoopCanvas layout |
|---|---|---|
| 1 | **Drums**: hits re-emitted into the `Oxygen Pro 61` port-1 input as channel-10 notes 36-51; blue at rest, green / yellow / red by hit | same |
| 2 | Mixer tracks 1-8: pads 1-8 mute (red / chartreuse), 9-16 solo (yellow / azure) | LoopCanvas press slot 1-8 (green) / drop slot 1-8 (azure) |
| 3 | Mixer tracks 9-16, same colours | LoopCanvas return section 1-8 (yellow) / align slot 1-8 (violet) |
| 4 | Markers & utilities: 1-8 go to marker 1-8 (orange); 9 insert marker (white); 10/11 previous/next marker (azure); 12 repeat (yellow when on); 13/14 undo/redo (violet); 15 save (green); 16 metronome (magenta when on) | Mixer tracks 1-8 |
| 5 | Free (white) | Free (white) |
